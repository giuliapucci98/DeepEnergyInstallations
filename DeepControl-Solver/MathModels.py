...
class Energy():
    ...
    def drift(self, t, hhx):
        h1, h2, x3 = torch.split(hhx, 1, dim=-1)
        drift1 = - self.xi[0]*h1
        drift2 = - self.xi[1]*h2
    ...
    def gamma(self, t, hhx, jump, control):
        '''
        ...
        '''
        h1, h2, x3 = torch.split(hhx, 1, dim=-1)
        gamma1 = torch.sum(self.sig[0,:] * jump, dim=-1, keepdim=True)
        gamma2 = self.sig[1,1]*jump[:,1:]

        dN = (jump != 0).float()  # check if there is a jump
        gamma3 = torch.sum(control*dN, dim=-1, keepdim=True)
        #gamma3 = torch.sum(control * jump, dim=-1, keepdim=True)
    ...
    def step_forward(self, i, x, jumps, control):
        x1, x2, x3 = torch.split(x, 1, dim=-1)
        hhx = torch.cat(((-torch.log(1-x1))/self.s, x2/self.d, x3), dim=-1)
        hhx_next = hhx + (self.drift(self.dt * i, hhx))*self.dt + self.gamma(self.dt * i, hhx, jumps, control)
        if (hhx[:,0:1] <= 0).any():
            print("non positive value encountered in h1 after drift and gamma computation.")
        x1_next = 1- torch.exp(-self.s * hhx_next[:, 0:1])
        x2_next = hhx_next[:, 1:2] * self.d
        x3_next = hhx_next[:, 2:3]
        return torch.cat((x1_next, x2_next, x3_next), dim=-1)
